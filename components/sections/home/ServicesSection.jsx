...
import { getServices } from '@/lib/data/home'
import { useLocale } from '@/lib/locale-context'

export default function ServicesSection() {
  const { locale, dict } = useLocale()
  const services = getServices(locale)
  const s = dict?.services_section || {}
  const prefix = locale === 'fr' ? '' : `/${locale}`
  const servicesHref = `${prefix}/services`

  const seeAll = locale === 'en' ? 'View all services' : locale === 'es' ? 'Ver todos los servicios' : 'Voir tous les services'
      ...
      <div className="wrap">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-0 md:mb-16">
          <motion.div
            ...
            className="max-w-xl"
          >
            <span className="badge mb-4">{s.badge || 'Nos Services'}</span>
            <h2
              className="text-3xl md:text-5xl font-bold leading-tight"
              style={{ fontFamily: 'var(--font-space, sans-serif)' }}
            >
              {s.h2_pre || 'Tout ce dont vous avez besoin'}{' '}
              <span className="text-gradient">{s.h2_highlight || 'pour dominer'}</span>
            </h2>
          </motion.div>
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <Link href={servicesHref} className="btn-outline">
              {seeAll} <ArrowRight size={16} />
            </Link>
          </motion.div>
        </div>
